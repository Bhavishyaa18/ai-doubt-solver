...
export default function Home() {
  ...
  const handleSolve = async (question: string, imageBase64?: string, mimeType?: string) => {
    setCurrentQuestion(question || "📷 Image question");
    setAppState("loading");
    setResult(null);
    setErrorMessage("");
    ...
    try {
      ...
      if (!data.success) throw new Error(data.error || "An unexpected error occurred.");
  ...
  const handleRetry = () => {
    if (currentQuestion) handleSolve(currentQuestion);
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 pb-20">
        ...
        <header className="pt-12 pb-10 text-center animate-fade-up">
            ...
            <div className="relative">
              <div className="w-14 h-14 rounded-2xl bg-ink-800 border border-amber-400/25 flex items-center justify-center amber-glow-strong">
                <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
                  <path d="M14 3L3 9v10l11 6 11-6V9L14 3z" stroke="#fbbf24" strokeWidth="1.5" strokeLinejoin="round"/>
                  <path d="M14 3v16M3 9l11 6 11-6" stroke="#fbbf24" strokeWidth="1.5" strokeLinejoin="round" strokeOpacity="0.5"/>
          ...
          <p className="text-slate-500 font-body text-base sm:text-lg max-w-md mx-auto leading-relaxed">
            ...
            <span className="text-amber-400/80">📷 upload a photo</span>{" "}
            from your textbook. Get instant{" "}
            <span className="text-slate-400">step-by-step answers.</span>
          </p>

          {/* Subject pills */}
          <div className="flex flex-wrap items-center justify-center gap-2 mt-5">
            {["📐 Maths", "⚡ Physics", "🧪 Chemistry", "🌱 Biology", "📜 History", "✍️ English", "💻 CS", "🌍 Geography"].map((subject) => (
              <span key={subject} className="text-xs font-display text-slate-600 bg-ink-800 border border-ink-700 px-2.5 py-1 rounded-full">
                {subject}
              </span>
            ))}
          </div>
        ...
        <section className="animation-delay-100 animate-fade-up">
          <InputBox onSolve={handleSolve} isLoading={appState === "loading"} />
        </section>
        ...
        <section ref={resultRef} className="mt-8">
          ...
          {appState === "result" && result && (
            <Result result={result} question={currentQuestion} onTryAnother={handleTryAnother} />
          )}
          {appState === "error" && (
        ...
        <footer className="mt-16 text-center animation-delay-300 animate-fade-up">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <span className="text-xs text-slate-700 font-mono px-3">AI Doubt Solver</span>
            <div className="h-px flex-1 bg-gradient-to-l from-transparent to-ink-700" />
          </div>
          <p className="text-xs text-slate-700 font-body">
            Built for students · All subjects · Type or upload photo · Free to use
          </p>
        </footer>
      </div>
    </main>
